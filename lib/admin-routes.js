...
// Keep the entire admin area out of search engines / archives.
...
const LOGIN_MAX = 8;            // attempts allowed
const LOGIN_WINDOW = 15 * 60e3; // per 15 minutes
const LOGIN_LOCK = 15 * 60e3;   // lockout duration
const loginHits = new Map();    // ip -> { count, first, lockedUntil }
function loginBlocked(ip) {
  const h = loginHits.get(ip);
  if (!h) return false;
  if (h.lockedUntil && Date.now() < h.lockedUntil) return true;
  if (h.lockedUntil && Date.now() >= h.lockedUntil) { loginHits.delete(ip); return false; }
  return false;
}
function loginFailed(ip) {
  const now = Date.now();
  let h = loginHits.get(ip);
  if (!h || now - h.first > LOGIN_WINDOW) h = { count: 0, first: now, lockedUntil: 0 };
  h.count++;
  if (h.count >= LOGIN_MAX) h.lockedUntil = now + LOGIN_LOCK;
  loginHits.set(ip, h);
}
...
setInterval(() => {
  const now = Date.now();
  for (const [ip, h] of loginHits) if (now - h.first > LOGIN_WINDOW && (!h.lockedUntil || now > h.lockedUntil)) loginHits.delete(ip);
}, 60e3).unref?.();
...
router.get('/login', (req, res) => {
  ...
  const err = req.query.err === 'locked'
    ? 'Too many attempts. Please try again in 15 minutes.'
    : (req.query.err ? 'Invalid username or password.' : '');
  res.send(r.layoutShell({
    title: 'Sign in', sidebar: false,
    body: `<div class="min-h-screen flex items-center justify-center bg-cream-50">
      <form method="POST" action="/admin/login" class="bg-white p-8 rounded-2xl ring-1 ring-ink-100 shadow-xl w-full max-w-sm" autocomplete="off">
        <img src="/heart-mark.png" alt="" width="48" height="48" class="w-12 h-12 mb-3">
        <h1 class="text-xl font-bold mb-6">UnifyHeart</h1>
        ${err ? r.flash('err', err) : ''}
        <label class="block text-xs font-bold uppercase tracking-wider text-ink-700 mb-1.5">Username</label>
...
router.post('/login', (req, res) => {
  ...
  if (loginBlocked(ip)) return res.status(429).redirect(302, '/admin/login?err=locked');
  const { username, password } = req.body;
  ...
  res.send(r.layoutShell({
    ...
    body: r.pageHeader('Broadcast', 'Send a one-off message to push subscribers.') + r.content(`
      ...
      <form method="POST" action="/admin/broadcast" class="max-w-2xl space-y-5 bg-white p-6 rounded-xl ring-1 ring-ink-100">
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-ink-700 mb-1.5">Channel</label>
          <select name="channel" class="w-full px-3 py-2 rounded-lg ring-1 ring-ink-100">
            <option value="both">Web push + iOS app</option>
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-ink-700 mb-1.5">Language</label>
          <select name="lang" class="w-full px-3 py-2 rounded-lg ring-1 ring-ink-100">
            <option value="en">English (${recipients.en} web subscribers)</option>
            <option value="tr">Türkçe (${recipients.tr} web subscribers)</option>
            <option value="fr">Français (${recipients.fr} web subscribers)</option>
          </select>
        </div>
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-ink-700 mb-1.5">Title</label>
...
router.post('/broadcast', async (req, res) => {
  const { lang, title, body, url, channel = 'both' } = req.body;
  if (!title) return flashRedirect(res, '/admin/broadcast', 'err', 'Title required.');
  const fakeArticle = { id: 0, lang, topic: 'rights', breaking: true, title, summary: body, image_url: null, url: url || `https://unifyheart.com/${lang}` };

  let web = 0, ios = 0;
  if (channel === 'both' || channel === 'web') {
    try { web = await pushBroadcast(fakeArticle); } catch {}
    db.prepare('INSERT INTO alert_log(channel, recipients, succeeded, failed, note) VALUES(?,?,?,?,?)')
      .run('push (manual)', web, web, 0, `lang=${lang} "${title.slice(0,60)}"`);
  }
  if (channel === 'both' || channel === 'ios') {
    try { ios = await apns.sendToAll(fakeArticle); } catch {}
    db.prepare('INSERT INTO alert_log(channel, recipients, succeeded, failed, note) VALUES(?,?,?,?,?)')
      .run('apns (manual)', ios, ios, 0, `lang=${lang} "${title.slice(0,60)}"`);
  }
  const parts = [];
  if (channel !== 'ios') parts.push(`${web} web`);
  if (channel !== 'web') parts.push(`${ios} iOS`);
  flashRedirect(res, '/admin/broadcast', 'ok', `Sent to ${parts.join(' + ')}.`);
});
