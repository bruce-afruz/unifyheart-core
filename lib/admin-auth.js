...
// Middleware. To keep the panel out of discovery, cold unauthenticated probes
// (no session cookie at all) get a plain 404 — the panel looks like it doesn't exist.
// Only a visitor with a *stale* session cookie (a real admin whose session expired)
...
function requireAdmin(req, res, next) {
  ...
  if (!user) {
    ...
    if (hadCookie && req.method === 'GET' && req.accepts && req.accepts('html')) {
