...
<footer class="bg-ink-900 text-cream-50 mt-0">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-14">
    ...
    <div class="mt-10 pt-6 border-t border-cream-50/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-cream-50/60">
      <span>© ${new Date().getFullYear()} UnifyHeart.com — ${esc(t(lang,'footer.love'))} · <a href="/${lang}/privacy" class="hover:text-heart-400 underline-offset-2 hover:underline">${lang === 'tr' ? 'Gizlilik' : lang === 'fr' ? 'Confidentialité' : 'Privacy'}</a></span>
...
const PRIVACY = {
  en: {
    ...
    updated: 'Last updated 24 May 2026',
    intro: 'UnifyHeart is a non-profit, volunteer-run project. We collect as little as possible, we never sell data, and we run no advertising or third-party trackers. This page explains exactly what we handle and why.',
    sections: [
      ['What we collect', [
        'Browser or app alerts: if you opt in, we store an anonymous push token (and your chosen language) so we can deliver alerts. It does not identify you personally.',
        'Email digests: if you subscribe by email, we store your email address, language and chosen frequency to send the digests you requested.',
        'Anonymous analytics: page visited, coarse country (derived from your network, not stored as an IP), device type and the referring site. No cookies, no cross-site tracking, no personal identifiers.',
      ]],
      ['What we never do', [
        'We never sell, rent or share your data with anyone.',
        'We run no advertising and no advertising trackers.',
        'We use no third-party analytics (no Google Analytics, no Meta pixel, etc.).',
        'Everything is stored on our own self-hosted server.',
      ]],
      ['How we use it', [
        'Push tokens and email addresses are used only to deliver the human-rights alerts you asked for.',
        'Anonymous analytics help us understand which stories reach people, so we can serve the mission better.',
      ]],
      ['Your choices', [
        'Unsubscribe from email any time via the one-click link in every message.',
        'Turn off browser or app notifications any time in your device settings.',
        'Email us to have any data we hold about you deleted.',
      ]],
      ['Children', ['UnifyHeart is not directed at children. We do not knowingly collect data from children.']],
      ['Contact', ['Questions or deletion requests: hello@unifyheart.com']],
    ],
  },
  tr: {
    title: 'Gizlilik Politikası',
    updated: 'Son güncelleme 24 Mayıs 2026',
    intro: 'UnifyHeart, gönüllülerle yürütülen kâr amacı gütmeyen bir projedir. Mümkün olan en az veriyi toplarız, veriyi asla satmayız ve hiçbir reklam veya üçüncü taraf takipçisi kullanmayız. Bu sayfa neyi neden işlediğimizi açıklar.',
    sections: [
      ['Neleri topluyoruz', [
        'Tarayıcı veya uygulama bildirimleri: izin verirsen, bildirim gönderebilmek için anonim bir bildirim anahtarı (ve seçtiğin dil) saklarız. Bu seni kişisel olarak tanımlamaz.',
        'E-posta özetleri: e-posta ile abone olursan, istediğin özetleri göndermek için e-posta adresini, dilini ve sıklığını saklarız.',
        'Anonim istatistikler: ziyaret edilen sayfa, kabaca ülke (ağdan türetilir, IP olarak saklanmaz), cihaz türü ve yönlendiren site. Çerez yok, siteler arası takip yok, kişisel tanımlayıcı yok.',
      ]],
      ['Asla yapmadıklarımız', [
        'Verini asla satmaz, kiralamaz veya kimseyle paylaşmayız.',
        'Hiçbir reklam ve reklam takipçisi kullanmayız.',
        'Üçüncü taraf istatistik aracı kullanmayız (Google Analytics, Meta pixel vb. yok).',
        'Her şey kendi sunucumuzda saklanır.',
      ]],
      ['Nasıl kullanıyoruz', [
        'Bildirim anahtarları ve e-posta adresleri yalnızca istediğin insan hakları bildirimlerini iletmek için kullanılır.',
        'Anonim istatistikler, hangi haberlerin insanlara ulaştığını anlamamıza ve misyona daha iyi hizmet etmemize yardımcı olur.',
      ]],
      ['Seçeneklerin', [
        'Her e-postadaki tek tıkla bağlantıyla istediğin an abonelikten çık.',
        'Tarayıcı veya uygulama bildirimlerini cihaz ayarlarından istediğin an kapat.',
        'Hakkındaki verilerin silinmesi için bize e-posta gönder.',
      ]],
      ['Çocuklar', ['UnifyHeart çocuklara yönelik değildir. Bilerek çocuklardan veri toplamayız.']],
      ['İletişim', ['Sorular veya silme talepleri: hello@unifyheart.com']],
    ],
  },
  fr: {
    title: 'Politique de confidentialité',
    updated: 'Dernière mise à jour le 24 mai 2026',
    intro: "UnifyHeart est un projet associatif et bénévole. Nous collectons le strict minimum, ne vendons jamais de données et n'utilisons aucune publicité ni traqueur tiers. Cette page explique précisément ce que nous traitons et pourquoi.",
    sections: [
      ['Ce que nous collectons', [
        "Notifications navigateur ou application : si vous y consentez, nous stockons un jeton de notification anonyme (et votre langue) pour vous envoyer des alertes. Il ne vous identifie pas personnellement.",
        "Résumés par e-mail : si vous vous abonnez, nous stockons votre adresse e-mail, votre langue et la fréquence choisie pour envoyer les résumés demandés.",
        "Statistiques anonymes : page visitée, pays approximatif (déduit du réseau, non stocké comme IP), type d'appareil et site référent. Aucun cookie, aucun suivi inter-sites, aucun identifiant personnel.",
      ]],
      ['Ce que nous ne faisons jamais', [
        'Nous ne vendons, ne louons ni ne partageons jamais vos données.',
        "Aucune publicité ni traqueur publicitaire.",
        "Aucun outil de statistiques tiers (pas de Google Analytics, pas de pixel Meta, etc.).",
        'Tout est hébergé sur notre propre serveur.',
      ]],
      ['Comment nous les utilisons', [
        "Les jetons de notification et les adresses e-mail servent uniquement à délivrer les alertes droits humains demandées.",
        "Les statistiques anonymes nous aident à comprendre quels récits touchent les gens, pour mieux servir la mission.",
      ]],
      ['Vos choix', [
        "Désabonnez-vous des e-mails à tout moment via le lien en un clic présent dans chaque message.",
        "Désactivez les notifications navigateur ou application à tout moment dans les réglages de votre appareil.",
        "Écrivez-nous pour faire supprimer les données vous concernant.",
      ]],
      ['Enfants', ["UnifyHeart ne s'adresse pas aux enfants. Nous ne collectons pas sciemment de données d'enfants."]],
      ['Contact', ['Questions ou demandes de suppression : hello@unifyheart.com']],
    ],
  },
};

function privacyPage({ lang, vapidPublic }) {
  const locale = locales[lang];
  const p = PRIVACY[lang] || PRIVACY.en;
  const body = `
${header(lang)}
<main class="max-w-3xl mx-auto px-5 sm:px-6 py-14 sm:py-20">
  <h1 class="font-serif text-3xl sm:text-5xl font-bold leading-tight">${esc(p.title)}</h1>
  <p class="mt-3 text-sm text-ink-700 dark:text-cream-100/60">${esc(p.updated)}</p>
  <p class="mt-6 text-lg leading-relaxed text-ink-700 dark:text-cream-100/85">${esc(p.intro)}</p>
  ${p.sections.map(([h, items]) => `
    <section class="mt-10">
      <h2 class="font-serif text-2xl font-bold">${esc(h)}</h2>
      <ul class="mt-4 space-y-2.5">
        ${items.map((it) => `<li class="flex gap-3 text-ink-700 dark:text-cream-100/80 leading-relaxed">
          <span class="mt-2 w-1.5 h-1.5 rounded-full bg-heart-500 flex-shrink-0"></span><span>${esc(it)}</span>
        </li>`).join('')}
      </ul>
    </section>`).join('')}
</main>
${footer(lang)}
`;
  return layout({ lang, locale, title: `${p.title} — UnifyHeart`, description: p.intro, body, vapidPublic });
}
...
module.exports = { homepage, articlePage, donatePage, confirmPage, privacyPage, esc };
